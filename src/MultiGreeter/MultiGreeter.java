...
public class MultiGreeter {
	...
	public MultiGreeter(int size) {
		...
		// If someone entered a number less than zero, use the default size.
		else {
			greetings = new String[DEFUALT_SIZE];
		}
		greetings[0] = "hello";
	...
	public int getNumGreetings() {
		// if no greetings have been added there should only be the default
		// return 1
	...
	public String greet(String name) {
		...
		if (defaultGreet) {
			...
			return (greetings[rand.nextInt(greetCount)] + " " + name);
		}
	}
	...
	public String greet() {
		...
		if (defaultGreet) {
			...
			return (greetings[rand.nextInt(greetCount)] + " World");
		}
	}
}